...
\begin{frame}{The case $m = 1$}
    \begin{proof}
        \begin{itemize}
            \item<1-> $H \cong H/\Phi(H)$ is a $\mathbb{F}_{p_1}$-vector space and thus $H \cong (\mathbb{Z}_{p_1})^q$
...
\begin{frame}{Properties of $L_k$}
    \begin{itemize}
        ...
        \item<6-> The sequence $d(L_k)_{k \in \mathbb{N}}$ is unbounded and non-decreasing.
        \item<7-> For all $k \in \mathbb{N}$, $d(L_{k+1}) \le d(L_{k})+1$.
    \end{itemize}
...
\begin{frame}{The function $f$}
    \begin{theorem}
        Let us assume that $M$ is abelian. Given a surjective homomorphism $\beta \colon L_k \rightarrow L/M$, the cardinality of the set $\mathscr{S}_\beta$ is $(q^k-1)/(q-1)$ where $q$ is the number of $(L/M)$-endomorphisms of $M$.
    \end{theorem}
    \begin{theorem}
...
\begin{frame}{The function $f$}
    ...
    \onslide<2->{\begin{theorem}
        Let $F$ be a free group of rank $m \ge d(L)$. Given a surjective homomorphism $\beta \colon F \rightarrow L/M$, the cardinality of the set $\mathscr{R}_{\beta}$ is $\phi_L(m)/\card{\Gamma}\phi_{L/M}(m)$, where $\Gamma$ denotes the set of all automorphisms of $L$ that act trivially on $L/M$. 
    \end{theorem}}
\end{frame}
